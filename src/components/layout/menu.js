...
const getMenus = function (menuArray, siderFold, authority, parentPath) {
  parentPath = parentPath || '/'
  return menuArray.map(item => {
    if(item.permission.indexOf(authority) !== -1 ) {
      if (item.child) {
        return (
...
function Menus ({ siderFold, darkTheme, location, isNavbar, handleClickNavMenu, authority }) {
  const menuItems = getMenus(menu, siderFold, authority)
  return (
    <Menu
